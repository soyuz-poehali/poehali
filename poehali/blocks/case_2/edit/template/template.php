...
function block_case_2($data)
{
	...
	switch ($content['style']) {
		case 1 : 
			include_once $_SERVER['DOCUMENT_ROOT'].'/blocks/case_2/edit/template/1/template.php';
			$case_out = block_case_2_1($data);
			break;		
	}

	return 
	'<div id="block_'.$data['id'].'" class="block" '.$style_container.' data-type="block" data-block="case_2" data-id="'.$data['id'].'">'.
		'<div class="e_block_panel">'.
			'<div class="e_block_panel_ico" data-action="item_add" title="Добавить кейс"><svg><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="/edit/blocks/template/e_sprite.svg#add"></use></svg></div>'.
		...
		'</div>'.
		'<div id="block_case_2_container_'.$data['id'].'" class="block_case_2_container dan_flex_row" data-id="'.$data['id'].'" '.$wrap_style.'>'.$case_out.'</div>'.
	'</div>';
}
?>