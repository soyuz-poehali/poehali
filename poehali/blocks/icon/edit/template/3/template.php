...
$SITE->setHeadFile('/blocks/icon/frontend/3/BLOCK.icon_3.css');

function block_icon_3($data)
{
	global $SITE;
	...
	foreach ($content['icons'] as $icon) {
		...
		$replace = 'svg class="block_icon_item_svg" style="fill:'.$content['icon_color'].';width:'.$content['icon_size'].'px;height:'.$content['icon_size'].'px;" '.$view_box;

		$svg_out = str_replace('svg ', $replace, $svg);
		
		if($content['icon_color'] == '#ffffff') 
			$hover_color = '#444444';
		else 
			$hover_color = $content['icon_color'];
		...
		$cpanel_item =
			'<div class="e_item_panel">'.
				...
				'<div class="drag_drop_ico" title="Перетащить" data-id="'.$data['id'].'" data-target-id="block_icon_container_'.$data['id'].'" data-class="block_icon_item_3" data-f="EDIT.block.icon.update_ordering"><svg><use xlink:href="/edit/blocks/template/e_sprite.svg#cursor_24"></use></svg></div>'.
			...
			$icons .=
				'<div class="block_icon_item_3 e_block_item" '.$item_style.' data-block="icon" data-id="'.$data['id'].'" data-item-num="'.$i.'">'.
					$cpanel_item.
					'<div class="block_icon_item_svg_wrap" style="border-color:'.$content['icon_color'].';">'.
						$svg_out.
					'</div>'.
					'<div>'.
