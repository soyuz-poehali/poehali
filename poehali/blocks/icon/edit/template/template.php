...
function block_icon($data)
{
	...
	$cpanel =
		'<div class="e_block_panel">'.
			'<div class="e_block_panel_ico" data-action="add" title="Добавить иконку"><svg><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="/edit/blocks/template/e_sprite.svg#add"></use></svg></div>'.
	...
	switch ($content['style']) {
		case 1 : 
			include_once $_SERVER['DOCUMENT_ROOT'].'/blocks/icon/edit/template/1/template.php';
			$icons = block_icon_1($data);
			break;		
		case 2 : 
			include_once $_SERVER['DOCUMENT_ROOT'].'/blocks/icon/edit/template/2/template.php';
			$icons = block_icon_2($data);
			break;
		case 3 : 
			include_once $_SERVER['DOCUMENT_ROOT'].'/blocks/icon/edit/template/3/template.php';
			$icons = block_icon_3($data);
			break;		
	}

	$html =
	'<div id="block_'.$data['id'].'" class="block" '.$style_container.' data-block="icon" data-id="'.$data['id'].'">'.
		$cpanel.
