...
function block_packages($data)
{
	...
	$cpanel =
		'<div class="e_block_panel">'.
			'<div class="e_block_panel_ico" data-action="package_add" title="Добавить пакет предложений"><svg><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="/edit/blocks/template/e_sprite.svg#add"></use></svg></div>'.
	...
	switch ($content['style']) {
		case 1 : 
			include_once $_SERVER['DOCUMENT_ROOT'].'/blocks/packages/edit/template/1/template.php';
			$packages = block_packages_1($data);
			break;		
		case 2 : 
			include_once $_SERVER['DOCUMENT_ROOT'].'/blocks/packages/edit/template/2/template.php';
			$packages = block_packages_2($data);
			break;
		case 3 : 
			include_once $_SERVER['DOCUMENT_ROOT'].'/blocks/packages/edit/template/3/template.php';
			$packages = block_packages_3($data);
			break;
		default: include_once $_SERVER['DOCUMENT_ROOT'].'/blocks/packages/edit/template/1/template.php';
			$packages = block_packages_1($data);
	}

	$out =
	'<div id="block_'.$data['id'].'" class="block" '.$style_container.' data-block="packages" data-id="'.$data['id'].'">'.
		$cpanel.
		'<div '.$wrap_style.'><div id="block_packages_container_'.$data['id'].'" class="dan_flex_center relative">'.$packages.'</div></div>'.
	'</div>';
