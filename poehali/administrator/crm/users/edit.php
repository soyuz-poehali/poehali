...
'<form method="post" action="/admin/administrators/'.$act.'" enctype="multipart/form-data">'.
	...
	'</div>'.
	'<div class="dan_flex_row">'.
		'<div class="a_tc_l">Пароль: '.$pass_tooltip.'</div>'.
		'<div class="a_tc_r">'.
			'<input class="dan_input" name="psw" type="password" '.$pass_required.' pattern="[a-z0-9_-]{8,20}" value="" title="Английские буквы в нижнем регистреб цифры, символы _ - от 8 до 20 символов">'.
		'</div>'.
	'</div>'.
	'<div class="dan_flex_row">'.
		'<div class="a_tc_l">Описание:</div>'.
		'<div class="a_tc_r">'.
			'<textarea rows="2" name="description" class="dan_input a_w_400">'.$administrator['description'].'</textarea>'.
		'</div>'.
	'</div>'.
	'<div class="dan_flex_row">'.
		'<div class="a_tc_l">Статус вкл/выкл:</div>'.
		'<div class="a_tc_r">'.
			'<input id="a_administrator_status" class="dan_input" name="status" type="checkbox" value="1" '.$status_check.'>'.
			'<label for="a_administrator_status"></label>'.
		'</div>'.
	'</div>'.
	'<div class="dan_flex_row a_m_40_0">'.
		'<div class="a_tc_l"><input class="dan_button_green" type="submit" name="submit" value="Сохранить"></div>'.
		'<div class="a_tc_r"><a href="/admin/administrators" class="dan_button_white">Отмена</a></div>'.
	'</div>'.
'</form>';
